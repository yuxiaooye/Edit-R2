...
class Bagel(PreTrainedModel):
    ...
    def forward_cache_update_text(
        ...
        # --- Update token_type: all new tokens are text ---
        new_len = past_key_values.key_cache[0].shape[0]
        _dev = past_key_values.key_cache[0].device
        merged_type = torch.zeros(new_len, dtype=torch.bool, device=_dev)
        if past_key_values.token_type is not None:
            merged_type[packed_key_value_indexes.to(_dev)] = past_key_values.token_type.bool().to(_dev)
        merged_type[packed_text_indexes.to(_dev)] = True
        past_key_values.token_type = merged_type
    ...
    def forward_cache_update_vit(
        ...
        # --- Update token_type: BOI/EOI are text, VIT tokens are image ---
        new_len = past_key_values.key_cache[0].shape[0]
        _dev = past_key_values.key_cache[0].device
        merged_type = torch.zeros(new_len, dtype=torch.bool, device=_dev)
        if past_key_values.token_type is not None:
            merged_type[packed_key_value_indexes.to(_dev)] = past_key_values.token_type.bool().to(_dev)
        merged_type[packed_indexes.to(_dev)[packed_text_indexes.to(_dev)]] = True
        past_key_values.token_type = merged_type
    ...
    def forward_cache_update_vae(
        ...
        # NOTE 
        # Move all CPU tensors to the model device (T2I never hits this path,
        # but image-editing conditioning images come in from CPU).
        ...
        # Disable autocast to avoid precision issues during VAE encoding
        with torch.amp.autocast(device_type='cuda', enabled=False):
            padded_latent = vae_model.float().encode(padded_images.to(device=vae_param.device).float())
        ...
        target_dtype = next(self.vae2llm.parameters()).dtype
        packed_latent = packed_latent.to(target_dtype)
        ...
        # --- Update token_type: BOI/EOI are text, VAE latent tokens are image ---
        new_len = past_key_values.key_cache[0].shape[0]
        _dev = past_key_values.key_cache[0].device
        merged_type = torch.zeros(new_len, dtype=torch.bool, device=_dev)
        if past_key_values.token_type is not None:
            merged_type[packed_key_value_indexes.to(_dev)] = past_key_values.token_type.bool().to(_dev)
        merged_type[packed_indexes.to(_dev)[packed_text_indexes.to(_dev)]] = True  # BOI/EOI markers
        # VAE positions stay False (image)
        past_key_values.token_type = merged_type
    ...
    def generate_image(
        ...
        sde_timestep_begin = random.randint(sample_sde_window_range[0], sample_sde_window_range[1] - sample_sde_window_size)
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            x_t, log_prob, _, _ = self._sde_step_with_logprob(
                v_t, 
                timesteps[i], 
                timesteps[i+1] if i+1 < len(timesteps) else timesteps[i]*0,
                dts[i], 
                x_t, 
    ...
    def generate_image_learn(
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=not accelerator.is_local_main_process):
            ...
            if t > cfg_interval[0] and t <= cfg_interval[1]:  # Apply CFG only within the specified interval
                cfg_text_scale_ = cfg_text_scale
                cfg_img_scale_ = cfg_img_scale
            else:
                cfg_text_scale_ = 1.0
            ...
            if grpo_config.train.beta > 0:
                v_t_policy = v_t  # save policy velocity before ref model overwrites
                with torch.no_grad():
                    v_t_ref = self._forward_flow(
                        x_t=latents[i],
                        timestep=timestep, 
                        packed_vae_token_indexes=packed_vae_token_indexes,
                        packed_vae_position_ids=packed_vae_position_ids,
                    ...
                    if not getattr(grpo_config.train, 'use_velocity_kl', False):
            ...
            if grpo_config.train.beta > 0:
                if getattr(grpo_config.train, 'use_velocity_kl', False):
                    kl_loss = ((v_t_policy - v_t_ref) ** 2).mean()  # velocity MSE (unweighted)
                else:
                    kl_loss = ((prev_sample_mean - prev_sample_mean_ref) ** 2).mean() / (2 * std_dev_t ** 2)  # latent KL
                loss = policy_loss + grpo_config.train.beta * kl_loss
            else:
                loss = policy_loss
            if i == 0 and pe_loss is not None:
                loss = loss + pe_loss
            _used_to_learn = sample.get("used_to_learn", None)
            if _used_to_learn is not None and not _used_to_learn.item():
                loss = loss * 0.0
            clipfrac.append(
                torch.mean(
                    (
    ...
    def _forward_flow(
        ...
        if cfg_text_scale > 1.0:
            cfg_text_output = forward_model.forward(
                packed_query_sequence=packed_sequence,
                query_lens=packed_seqlens,
        ...
        if cfg_img_scale > 1.0:
            cfg_img_output = forward_model.forward(
                packed_query_sequence=packed_sequence,
                query_lens=packed_seqlens,
    ...
    def generate_text(
        ...
        while step < max_length:
            ...
            pred_logits = self.language_model.forward(mode="get_logits", hidden_state=packed_query_sequence)

            if do_sample:
                probs = nn.functional.softmax(pred_logits / temperature, dim=-1)
                curr_tokens = torch.multinomial(probs, num_samples=1).squeeze(1)
                if return_log_probs:
                    # Use log_softmax to compute log_prob, consistent with generate_text_learn
                    log_probs_for_gather = nn.functional.log_softmax(pred_logits / temperature, dim=-1)
                    token_log_prob = log_probs_for_gather.gather(-1, curr_tokens.unsqueeze(-1)).squeeze(-1)
                    log_probs_list.append(token_log_prob)
            else:
                curr_tokens = torch.argmax(pred_logits, dim=-1)
                if return_log_probs:
                    log_probs_of_argmax = nn.functional.log_softmax(pred_logits, dim=-1)
                    token_log_prob = log_probs_of_argmax.gather(-1, curr_tokens.unsqueeze(-1)).squeeze(-1)
                    log_probs_list.append(token_log_prob)
            ...
            if end_token_id is not None and curr_tokens[0] == end_token_id:
                if not pad_to_max_length:
                    break
                if eos_step is None:
                    eos_step = len(generated_sequence)

        output_device = generated_sequence[0].device
        if pad_to_max_length and eos_step is not None:
            seq = torch.stack([i.to(output_device) for i in generated_sequence[:eos_step]], dim=0)
        else:
            seq = torch.stack([i.to(output_device) for i in generated_sequence], dim=0)

        if return_log_probs:
            # log_probs[i] = log P(next_token | context, seq[:i+1])
            # length = len(seq) - 1 when truncated at eos, else len(seq)
            trunc = eos_step if (pad_to_max_length and eos_step is not None) else len(log_probs_list)
            lp = torch.stack([i.to(output_device) for i in log_probs_list[:trunc]], dim=0)
            return seq, lp
        return seq

    def generate_text_learn(
        self,
        past_key_values: NaiveCache,
        packed_key_value_indexes: torch.LongTensor,
        key_values_lens: torch.IntTensor,
        ...
        """
        Teacher-forced forward: given a known token sequence, compute new log_prob for all tokens
        in a single forward pass with is_causal=True (same mode as forward_cache_update_text),
        avoiding activation checkpointing conflicts caused by per-token KV cache modifications.

        Args:
            past_key_values: KV cache of the context (system prompt + question already encoded).
            packed_key_value_indexes: KV cache indexes.
            key_values_lens: KV cache lengths.
            packed_query_position_ids: Starting position id (scalar tensor).
            token_ids: Stored token sequence [bos, tok1, ..., tokK], shape (seq_len,).
            token_len: Actual valid length (excluding padding).
            temperature: Temperature consistent with the sampling phase.

        Returns:
            per_token_log_probs: shape (token_len - 1,),
                log_probs[i] = log P(token_ids[i+1] | context, token_ids[:i+1])
        """
        valid_ids = token_ids[:token_len]  # (token_len,)
        device = valid_ids.device

        # Build input for a single forward pass (cf. prepare_prompts + forward_cache_update_text)
        kv_len = key_values_lens[0].item()  # batch=1
        start_pos = packed_query_position_ids[0].item()

        # position ids: [start_pos, start_pos+1, ..., start_pos+token_len-1]
        packed_text_position_ids = torch.arange(
            start_pos, start_pos + token_len, dtype=torch.long, device=device
        )
        # query indexes: positions in the merged KV+query space
        packed_text_indexes = torch.arange(
            kv_len, kv_len + token_len, dtype=torch.long, device=device
        )

        # Get embeddings for all tokens
        packed_text_embedding = self.language_model.forward(
            mode="get_embeddings", input_ids=valid_ids
        )
        ...
        # Single forward pass; is_causal=True enforces the causal mask
        output = self.language_model.forward(
            packed_query_sequence=packed_text_embedding,
            query_lens=torch.tensor([token_len], dtype=torch.int, device=device),
            packed_query_position_ids=packed_text_position_ids,
            packed_query_indexes=packed_text_indexes,
            past_key_values=past_key_values,
            key_values_lens=key_values_lens,
            packed_key_value_indexes=packed_key_value_indexes,
            update_past_key_values=False,  # Do not modify KV cache to avoid activation checkpointing conflicts
            is_causal=True,
            **extra_inputs,
        )

        # Get logits at all positions: (token_len, vocab_size)
        all_logits = self.language_model.forward(
            mode="get_logits", hidden_state=output.packed_query_sequence
        )

        # Compute shifted log_probs: logits[i] predicts token_ids[i+1]
        log_probs_all = nn.functional.log_softmax(all_logits[:-1] / temperature, dim=-1)  # (token_len-1, vocab)
        target_ids = valid_ids[1:]  # (token_len-1,)
        per_token_log_probs = log_probs_all.gather(-1, target_ids.unsqueeze(-1)).squeeze(-1)  # (token_len-1,)
