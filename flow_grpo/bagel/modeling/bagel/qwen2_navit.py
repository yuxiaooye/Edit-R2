...
from flow_grpo.bagel.modeling.qwen2.modeling_qwen2 import (
    Qwen2Attention, 
    Qwen2MLP, 
...
from flow_grpo.bagel.modeling.qwen2.configuration_qwen2 import Qwen2Config as _Qwen2Config
...
class NaiveCache:
    def __init__(self, num_layers):
        ...
        self.token_type = None  # 1D bool tensor: True=text, False=image. Shared across layers.
...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward_inference(
        ...
        if mode == 'und':
            ...
            # if packed_query_sequence.requires_grad:
            #     breakpoint()
            #     a=1
        ...
        # --- Text Key Scaling: boost attention to text keys during denoising ---
        _text_key_scale = getattr(self.config, 'text_key_scale', 1.0)
        if mode == 'gen' and _text_key_scale != 1.0 and not update_past_key_values:
            _total_kv = merged_key_states.shape[0]
            _text_mask = torch.zeros(_total_kv, dtype=torch.bool, device=merged_key_states.device)
            if past_key_values is not None and past_key_values.token_type is not None:
                _text_mask[packed_key_value_indexes] = past_key_values.token_type.bool()
            if packed_text_indexes is not None and packed_text_indexes.numel() > 0:
                _text_mask[packed_query_indexes[packed_text_indexes]] = True
            if _text_mask.any():
                merged_key_states[_text_mask] = merged_key_states[_text_mask] * _text_key_scale
