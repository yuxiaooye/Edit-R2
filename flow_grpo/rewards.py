...
# def ocr_edit_score(device):
#     from flow_grpo.ocr import OcrEditScorer

#     scorer = OcrEditScorer()
...
def edival_client_score(device, server_url=None):
    """EDiVal Instruction Following Reward Score (client mode).

    Calls the remote EDiVal Reward Server via HTTP.
    Suitable for service deployment to save GPU memory on the training host.

    Environment variables:
        EDIVAL_SERVER_URL: server address (default: http://localhost:12342)

    Usage in config:
        reward_fn = {"edival_client": 1.0}

    Start the server:
        python reward_server/edival_reward_server.py
    """
    ...
    def _fn(images, prompts, metadata, ref_images=None, base_images=None, only_strict=True,
        ...
        for score_name, weight in score_dict.items():
            if score_name == "geneval":
                ...
                # EDiVal requires ref_images to compare before/after editing
                ...
                print(f'[cc bg debug] In multi_score, prompts={prompts}, metadata={metadata} (check bg_consistency)')
                ...
                scores, rewards = score_fns[score_name](cu_images_history, cu_prompts_history, cu_format_instructions_history)
